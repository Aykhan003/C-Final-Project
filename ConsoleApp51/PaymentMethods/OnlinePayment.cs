...
internal class OnlinePayment : IPaymentService
{
    private PaymentStatus _paymentStatus = PaymentStatus.Pending;
    public void Pay(int orderId, decimal amount)
    {
        Console.WriteLine($"Processing online payment for Order ID: {orderId}, Amount: {amount}");
    ...
    public void Refund(int orderId, decimal amount)
    {
        ...
        {
            Console.WriteLine($"Processing online refund for Order ID: {orderId}, Amount: {amount}");
